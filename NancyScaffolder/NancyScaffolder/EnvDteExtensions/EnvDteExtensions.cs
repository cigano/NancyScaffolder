...
namespace NancyScaffolder.EnvDteExtensions
{
    public static class EnvDteExtensions
    {
        #region Language info
        // Support C# and VB syntax
        private static readonly Regex UnwrapNullableRegex = new Regex(@"^System.Nullable(<|\(Of )(?<UnderlyingTypeName>.*)(>|\))$");

        private static readonly Dictionary<string, string> CsharpShortenedNames = new Dictionary<string, string> {
            { "System.SByte", "sbyte" },
            { "System.Byte", "byte" },
            { "System.Int16", "short" },
        ...
        private static bool IsType(string languageSpecificLongTypeName, Type type, string languageGuid)
        {
            ...
            var codeDomProvider = string.Equals(languageGuid, VsConstants.VbCodeModelLanguageGuid, StringComparison.OrdinalIgnoreCase)
